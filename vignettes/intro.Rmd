...
First, we load the package and the `TravelTime2011` dataset used in the paper. This dataset contains 52 rows (estimates for each of the 50 US states and Washington, D.C., as well as a national-level estimate) and 7 columns. The variables `Estimate.2dec` and `SE.2dec` are estimates of the mean and its standard error for the mean travel time (in minutes) to work of workers 16 years and over who did not work at home, from the 2011 American Community Survey (ACS).
